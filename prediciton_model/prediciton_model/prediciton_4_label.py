...
for idx, label_group in enumerate(label_groups):
    csvfile = open(f'result_label_group_{idx + 1}.csv', 'w', newline='')
...
for image in data['Image']:
    for idx, label_group in enumerate(label_groups):
        ...
        max_index = prediction_results.argmax()
        prediction = seasons[max_index]
        confidence = prediction_results[0, max_index]
        # Write the prediction result to the corresponding CSV file
        csv_writers[idx].writerow({'Image': image, 'Prediction': prediction, 'Confidence': confidence})
